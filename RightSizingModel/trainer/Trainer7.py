...
class Trainer7():
    ...
    def train(self, reduce='sum'):  
        ...
        for epoch in range(self.epochs):
            ...
            epoch_loss_dict = {"epoch": epoch, "epochs": self.epochs, "Loss": np.sum(loss)/len_dataset, "time": time.time() - current_time,
                               ...
                               "Chest Loss": np.sum(loss_dict["Chest Loss"])/len_dataset, "Laplacian Loss": np.sum(loss_dict["Laplacian Loss"])/len_dataset}
    ...
    def train_epoch(self, reduce='mean'):
        ...
        for idx, data in enumerate(self.train_loader, 0):
            ...
            loss_list = [0.01 * (recon_loss + kl_divergence), 1000 * height_loss, 1000 * waist_loss, 1000 * chest_loss, laplacian_loss]
