...
20231124-185717: We suspect the chest circumference is not correlated to the waist circumference. 
                 (In fact, this one has better result.)
                 This means 
...
class Trainer1():
    ...
    def train(self, reduce='mean'):  
        ...
        for epoch in range(self.epochs):
            ...
            epoch_loss_dict = {"epoch": epoch, "epochs": self.epochs, "Loss": np.sum(loss)/len_dataset, "time": time.time() - current_time,
                               ...
                               "Waist Loss": np.sum(loss_dict["Waist Loss"])/len_dataset}
    ...
    def train_epoch(self, alpha, beta, reduce='mean'):
        ...
        epoch_height_losses, epoch_waist_loss, epoch_chest_loss = [], [], []
        ...
        for idx, data in enumerate(self.train_loader, 0):
            ...
            # Compute the gradient of height/chest/waist w.r.t. z
            ...
            loss = alpha * recon_loss + beta * (kl_divergence + 100 * height_loss + 100 * waist_loss + 100 * chest_loss)       
            ...
            epoch_waist_loss.append(beta * 100 * waist_loss.item() * x.size(0))
            epoch_chest_loss.append(beta * 100 * chest_loss.item() * x.size(0))
        ...
        loss_dict = {"Loss": epoch_losses, "Recon Loss": epoch_recon_losses, "KL Loss": epoch_kl_losses, 
                     "Height Loss": epoch_height_losses, "Waist Loss": epoch_waist_loss, "Chest Loss": epoch_chest_loss} 
