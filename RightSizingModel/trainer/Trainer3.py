...
class Trainer3():
    ...
    def train(self, reduce='mean'):  
        ...
        for epoch in range(self.epochs):
            ...
            epoch_loss_dict = {"epoch": epoch, "epochs": self.epochs, "Loss": np.sum(loss)/len_dataset, "time": time.time() - current_time,
                               ...
                               "Lap Loss": np.sum(loss_dict["Lap Loss"])/len_dataset}
    ...
    def train_epoch(self, alpha, beta, reduce='mean'):
        ...
        epoch_height_losses, epoch_waist_losses = [], []
        ...
        for idx, data in enumerate(self.train_loader, 0):
            ...
            height_loss = torch.norm(height_grad[:, 1:], p=2) / x.size(0)
            waist_loss = torch.norm(torch.concat((waist_grad[:, 0:1], waist_grad[:, 2:]), dim=1), p=2) / x.size(0)
            
            laplacian_loss = diff_laplacian(out)

            
            loss = alpha * recon_loss + beta * (kl_divergence + 100 * height_loss + 100 * waist_loss + 0.1 * laplacian_loss)      
            ...
            epoch_Lap_losses.append(beta * 0.1 * laplacian_loss.item() * x.size(0))
        ...
        loss_dict = {"Loss": epoch_losses, "Recon Loss": epoch_recon_losses, "KL Loss": epoch_kl_losses, 
                     "Height Loss": epoch_height_losses, "Waist Loss": epoch_waist_losses, "Lap Loss": epoch_Lap_losses} 
