...
class Trainer16():
    ...
    def train(self, reduce='sum'):  
        ...
        for epoch in range(self.epochs):
            ...
            epoch_loss_dict = {"epoch": epoch, "epochs": self.epochs, "Loss": np.sum(loss)/len_dataset, "time": time.time() - current_time,
                               ...
                               "Chest Loss": np.sum(loss_dict["Chest Loss"])/len_dataset, "Hip Loss": np.sum(loss_dict["Hip Loss"])/len_dataset,
    ...
    def train_epoch(self, reduce='mean'):
        ...
        epoch_height_losses, epoch_waist_loss, epoch_chest_loss, epoch_hip_loss = [], [], [], []
        epoch_arm_loss, epoch_crotch_loss = [], []
        ...
        for idx, data in enumerate(self.train_loader, 0):
            ...
            height_loss = torch.norm(height_grad[:, 1:], p=2) ** 2
            waist_loss = torch.norm(torch.concat((waist_grad[:, 0:1], waist_grad[:, 2:]), dim=1), p=2) ** 2
            chest_loss = torch.norm(torch.concat((chest_grad[:, 0:2], chest_grad[:, 3:]), dim=1), p=2) ** 2 
            hip_loss = torch.norm(torch.concat((hip_grad[:, 0:3], hip_grad[:, 4:]), dim=1), p=2) ** 2
            arm_loss = torch.norm(torch.concat((arm_grad[:, 0:4], arm_grad[:, 5:]), dim=1), p=2) ** 2
            crotch_loss = torch.norm(torch.concat((crotch_grad[:, 0:5], crotch_grad[:, 6:]), dim=1), p=2) ** 2
            ...
            epoch_hip_loss.append(1000 * hip_loss.item())
            epoch_arm_loss.append(1000 * arm_loss.item())
            epoch_crotch_loss.append(1000 * crotch_loss.item())
            ...
            wandb.log({"Hip Loss": 1000 * hip_loss.item()})
            wandb.log({"Arm Loss": 1000 * arm_loss.item()})
            wandb.log({"Crotch Loss": 1000 * crotch_loss.item()})
        ...
        loss_dict = {"Loss": epoch_losses, "Recon Loss": epoch_recon_losses, "KL Loss": epoch_kl_losses, 
                     ...
                     "Chest Loss": epoch_chest_loss, "Hip Loss": epoch_hip_loss,
                     "Arm Loss": epoch_arm_loss, "Crotch Loss": epoch_crotch_loss,
