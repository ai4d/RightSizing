...
class PCA_COMA(nn.Module):
    def __init__(self, in_channels, out_channels, latent_channels, edge_index,
        ...
        self.en_linear = nn.Linear(latent_channels, latent_channels, bias=False)
        self.linear_weight = self.en_linear.weight
        ...
        # this is what I added...
        with torch.no_grad():
            self.de_linear = nn.Linear(latent_channels, latent_channels, bias=False)
            self.de_linear.weight = nn.Parameter(self.linear_weight.T)
